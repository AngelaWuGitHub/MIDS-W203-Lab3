...
```{r}
############################################ Redo this with heteroskedasticity consistent standard errors
...
# Model Building 3
```{r}
model5 <- lm(crmrte_log ~ prbarr+prbconv+prbpris+avgsen+polpc_log+density_log+taxpc+west+central+urban+pctmin80_2+wcon+wtuc+wtrd+wfir+wser+wmfg+wfed+wsta+wloc+mix+pctymle, data=crimeData2)
summary(model5)
plot(model5)
AIC(model5)
```
